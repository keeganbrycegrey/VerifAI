...
async function _callBackend(payload) {
    console.log(`[VerifAI] Calling backend at ${API_BASE_URL}/check`, payload)
    try {
        ...
        console.log(`[VerifAI] Backend response status: ${response.status}`)
        if (!response.ok) {
            const errText = await response.text()
            console.error(`[VerifAI] Backend error: ${response.status} - ${errText}`)
            throw new Error(`backend error: ${response.status}`)
        }
        const data = await response.json()
        console.log(`[VerifAI] Backend returned verdict for claim: ${data.claim}`)
        _saveVerdictToHistory(data)
        return data
    } catch (error) {
        console.error(`[VerifAI] Backend call failed:`, error)
        throw error
    }
}
